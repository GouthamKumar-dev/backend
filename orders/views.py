...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        total_price = sum((item.product.offer_price if item.product.offer_price else item.product.price) * item.quantity for item in cart_items)
        ...
        try:
            ...
            for item in cart_items:
                OrderDetail.objects.create(
                    ...
                    price_at_purchase=item.product.offer_price if item.product.offer_price else item.product.price
                )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        return Response({
            "order_id": order.order_id,
            "total_price":  sum(detail.product.offer_price if detail.product.offer_price else detail.product.price for detail in order.order_details.all()),
            "status": order.status,
